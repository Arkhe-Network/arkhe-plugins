...
use safe_core_policy::barrier::{BarrierChecker, BarrierVerdict};
use safe_core_policy::ledger::{FailureLedger, DeflationClass, FailureEntry};
use safe_core_policy::trust_tier::TrustedArtifact;
    ...
    pub fn evaluate(&mut self, proposal: Strategy) -> Result<OrchestratorDecision, OrchestratorError> {
        ...
        match triage {
            ...
            TriageVerdict::Novel { forecast } => {
                ...
                if let BarrierVerdict::Barred { model, reason, confidence: _ } = barrier {
                    // Registra como falha
                    let entry = FailureEntry {
                        strategy_id: proposal.id.clone(),
                ...
                // 4. Consome orçamento
                self.budget.consume(proposal.estimated_cost).map_err(|e| OrchestratorError::Internal(e.to_string()))?;
