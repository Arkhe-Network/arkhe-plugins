...
mod tests {
    ...
    fn test_lemma_32_c2_sharp() {
        ...
        let bound = inertia.certify_c2().unwrap_or(0);
    ...
    fn test_lemma_32_general() {
        ...
        let (p, q) = CoherenceInertia::spectral_split(&matrix, 1.9).unwrap();
        let b_observed = CoherenceInertia::positive_index(&q);
        // b_observed deve ser 8 (todos menos os -1 são > 0)
        // Wait, looking at spectral_split with threshold 1.9, values 2.0, 1.9, 1.8... (indices 5, 6, 7). Wait:
        // i=0: 1.0, i=1: 1.1, i=2: 1.2, i=3: 1.3, i=4: 1.4 -> Q (<= 1.9)
        // i=5: 2.0 -> P (> 1.9)
        // i=6: 1.9 -> Q (<= 1.9)
        // i=7: 1.8 -> Q
        // i=8,9: -1.0 -> Q
        assert_eq!(b_observed, 8);

        let inertia = CoherenceInertia::new(p.clone(), q.clone(), b_observed).unwrap();

        // c=2
        let bound_c2 = inertia.certify(2.0).unwrap_or(0);

        // c=3 (deve ser mais fraco ou igual)
        let bound_c3 = inertia.certify(3.0).unwrap_or(0);
        // Como c=3 tem coeficientes menores, pode dar um bound menor
        // mas ainda deve ser ≥ 0

        // Cauchy-Schwarz (fallback)
        let bound_cs = inertia.certify_cauchy_schwarz().unwrap_or(0);
    }
