...
use crate::policy::ledger::{DeflationClass, FailureEntry, FailureLedger};
use crate::policy::trust_tier::{
    TheoremClaim, TrustedArtifact,
};
...
impl ValidationBudget {
    ...
    pub fn can_afford(&self, cost: ValidationCost) -> bool {
        match cost {
            ...
            ValidationCost::Composite(costs) => costs.iter().all(|c| self.can_afford(*c)),
        }
    }
...
impl Level25Orchestrator {
    ...
    pub fn evaluate(
        &mut self,
        proposal: Strategy,
    ) -> Result<OrchestratorDecision, OrchestratorError> {
...
impl Default for MetaCognitiveTriage {
    fn default() -> Self {
        Self::new()
    }
}
...
impl MetaCognitiveTriage {
    ...
    pub fn classify(&self, proposal: &Strategy) -> TriageVerdict {
        ...
        if proposal.claim.statement.contains("Riemann Hypothesis") {
            ...
            TriageVerdict::Novel {
                forecast: 0.6 + 0.3 * proposal.claim.confidence,
            }
        }
    }
}
...
pub struct TrustRegistry {
    _artifacts: Vec<TrustedArtifact<TheoremClaim>>,
}
