    ...
    pub fn promote(&mut self, validator: ValidatorInfo) -> Result<(), TrustError> {
        ...
        if let Some(target) = next_tier {
            ...
            self.audit_trail.push(AuditEntry {
                from_tier: old_tier,
                to_tier: target,
                reason: format!("Promoção por validador: {}", validator.name),
...
mod tests {
    ...
    fn test_minimum_for_use() {
        assert_eq!(
            TrustTier::minimum_for_use(ArtifactUsage::Research),
            TrustTier::InternalOnly
        );
        assert_eq!(
            TrustTier::minimum_for_use(ArtifactUsage::Prototype),
            TrustTier::CrossModel
        );
        assert_eq!(
            TrustTier::minimum_for_use(ArtifactUsage::Simulation),
            TrustTier::SymbolicVerified
        );
        assert_eq!(
            TrustTier::minimum_for_use(ArtifactUsage::Production),
            TrustTier::FormalVerified
        );
        assert_eq!(
            TrustTier::minimum_for_use(ArtifactUsage::SafetyCritical),
            TrustTier::HumanAudited
        );
    }
